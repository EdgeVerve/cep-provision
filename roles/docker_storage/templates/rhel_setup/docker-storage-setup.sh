...
# This section reads the config file ./docker-storage-setup.conf
...
wait_for_dev() {
  ...
  if [ -b "$devpath" ];then
    echo "Device node $devpath exists."
    return 0
  fi

  if [ -z "$DEVICE_WAIT_TIMEOUT" ] || [ "$DEVICE_WAIT_TIMEOUT" == "0" ];then
    echo "Not waiting for device $devpath as DEVICE_WAIT_TIMEOUT=${DEVICE_WAIT_TIMEOUT}."
    return 0
  fi

  while [ $timeout -gt 0 ]; do
    echo "Waiting for device $devpath to be available. Wait time remaining is $timeout seconds"
  ...
  echo "Timed out waiting for device $devpath"
  return 1
}
...
create_datadisk_lv() {
  if [ ! -n "$DATADISK_SIZE" ]; then
    Fatal "Data volume creation failed. No DATA_SIZE specified"
  fi

  if ! check_data_size_syntax $DATADISK_SIZE; then
    Fatal "DATADISK_SIZE value $DATADISK_SIZE is invalid."
  fi

  check_min_data_size_condition

  # TODO: Error handling when DATA_SIZE > available space.
  if [[ $DATADISK_SIZE == *%* ]]; then
    lvcreate -y -l $DATADISK_SIZE -n $DATADISK_LV_NAME $VG
  else
    lvcreate -y -L $DATADISK_SIZE -n $DATADISK_LV_NAME $VG
  fi
  
  mkfs -t ext4 /dev/$VG/$DATADISK_LV_NAME
  
  mkdir -p $DATADISK_PATH
  
  mount -t ext4 /dev/$VG/$DATADISK_LV_NAME $DATADISK_PATH
}
...
check_docker_storage_metadata() {
  ...
  echo "Docker state can be reset by stopping docker and by removing ${DOCKER_METADATA_DIR} directory. This will destroy existing docker images and containers and all the docker metadata."
  exit 1
}
...
setup_lvm_thin_pool () {
  ...
  if [ -n "$tpool" ]; then
     ...
     echo "Found an already configured thin pool $tpool in ${DOCKER_STORAGE}"
  ...
  if ! lvm_pool_exists; then
    ...
    if [ ! -f "$DOCKER_STORAGE" ];then
      echo "$DOCKER_STORAGE file is missing. Recreating it."
      write_storage_config_file
    fi
  fi
...
check_wipe_block_dev_sig() {
  ...
  if [ "$WIPE_SIGNATURES" == "true" ];then
    echo "Wipe Signatures is set to true. Any signatures on $bdev will be wiped."
...
scan_disks() {
  ...
  for dev in $DEVS_ABS; do
    ...
    if is_dev_part_of_vg ${dev}1 $VG; then
      echo "Device ${dev} is already partitioned and is part of volume group $VG"
      continue
    fi
...
elif [ -e {{cepfolder}}/cep/docker-storage-setup/libdss.sh ]; then
  source {{cepfolder}}/cep/docker-storage-setup/libdss.sh
fi

# If user has overridden any settings in ./docker-storage-setup.conf
# take that into account.
if [ -e {{cepfolder}}/cep/docker-storage-setup/docker-storage-setup.conf ]; then
  source {{cepfolder}}/cep/docker-storage-setup/docker-storage-setup.conf
fi

# Check if the volume group already exists
echo volume_group= $VG
VGS_EXISTING=$(vgs --nameprefixes --noheadings -o vg_name $VG 2>/dev/null || echo "")
echo VGS_EXISTING=$VGS_EXISTING
if [[ ! -z $VGS_EXISTING ]]; then
  echo "Volume group already exists..exiting"
  exit 0
fi
...
if ! ROOT_VG=$(lvs --noheadings -o vg_name $ROOT_DEV 2>/dev/null);then
  echo "Volume group backing root filesystem could not be determined"
  ROOT_VG=
else
